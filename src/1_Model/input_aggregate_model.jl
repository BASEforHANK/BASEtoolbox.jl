...
F[indexes.firm_profits] = log.(firm_profits) - log.(Y .* (1.0 - mc) .+ q .* (KPrime .- (1.0 .- depr) .* K) .- I)    
F[indexes.profits] = log.(profits) - log.((1.0 .- ωΠ) .* firm_profits .+ ιΠ .* (qΠ .- 1.0)) # distributed profits to entrepreneurs
F[indexes.qΠ] = log.(RBPrime ./ πPrime) .- log.(((qΠPrime .- 1.0) .* (1 - ιΠ) .+ ωΠ .* firm_profitsPrime) ./ (qΠ .- 1.0))

F[indexes.RL] = log.(RL) - log.((RB .* Bgov .+ π .* ((qΠ .- 1.0) .* (1 - ιΠ) .+ ωΠ .* firm_profits)) ./ B)
