...
@set! n_par.n_agg_eqn = 29
 #no replication
# Set aggregate steady state variabel values
ZSS = m_par.Z
mc = 1.0 ./ m_par.μ
wFSS = wage(mc, ZSS, KSS, NSS, m_par)
...
function equishares(x1, x2, grid_i, distr_i, nx)
    FN_Wshares = cumsum((grid_i .+ minimum(grid_i)) .* distr_i) ./ dot((grid_i .+ minimum(grid_i)), distr_i)
