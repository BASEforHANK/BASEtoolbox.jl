...
TSS =
    (TbarSS .- 1.0) .* (1.0 ./ m_par.μw .* wFSS .* NSS) + # labor income
    (TbarSS .- 1.0) .* Π_ESS + # profit income
    (TbarSS .- 1.0) * ((1.0 .- 1.0 ./ m_par.μw) .* wFSS .* NSS) # union profit income

RLSS = RBSS
RDSS = RRDSS .* πSS

qΠSS = m_par.ωΠ .* (1.0 .- 1.0 ./ m_par.μ) .* YSS ./ (RRLSS .- 1 .+ m_par.ιΠ) + 1.0
BgovSS = BSS .- qΠSS .+ 1.0
