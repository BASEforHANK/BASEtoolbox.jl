...
@doc raw"""
    prepare_linearization(KSS, WbSS, WkSS, distrSS, n_par, m_par)
...
- `WbSS::Array{Float64,3}`, `WkSS::Array{Float64,3}`: steady-state marginal value functions
...
- `CDFSS`, `CDF_b`, `CDF_k`, `CDF_h`: cumulative distribution functions (joint and
  marginals)
...
function prepare_linearization(
    ...
    # Solution to household problem in the stationary equilibrium, given prices
    KSS,
    BSS,
    ΓSS,
    Γ_aSS,
    ...
    compressionIndexes[2] = compressionIndexesWk # Technically, for the OneAsset case, this should be empty ... since the only coefficient generated is immutable ... sqrt(nb*nk*nh) ... but yea ...
    ...
    @set! n_par.copula_marginal_b = n_par.nb == 1 ? distr_bSS :
        copula_marg_equi(distr_bSS, n_par.grid_b, n_par.nb_copula)
    @set! n_par.copula_marginal_k =
        n_par.nk == 1 ? distr_kSS : copula_marg_equi(distr_kSS, n_par.grid_k, n_par.nk_copula)
    @set! n_par.copula_marginal_h = n_par.nh == 2 ? distr_hSS :
...
TSS =
    τbarSS .* (1.0 ./ m_par.μw .* wFSS .* NSS) + # labor income
    τbarSS .* Π_ESS + # profit income
    τbarSS * ((1.0 .- 1.0 ./ m_par.μw) .* wFSS .* NSS) # union profit income
...
qΠSS =
    (RRLSS .- 1 .+ m_par.ιΠ) == 0.0 ? 1.0 :
    m_par.ωΠ .* Π_FSS ./ (RRLSS .- 1 .+ m_par.ιΠ) + 1.0
...
CompCSS = (CSS - NSS.^(1+m_par.γ)/(1+m_par.γ))
    ...
    for (key, value) in pairs(store_these)
        if !isapprox(exp(XSS[getfield(indexes, Symbol(key))]), value)
            @warn "Inconsistency detected for $key: expected $value, got $(exp(XSS[getfield(indexes, Symbol(key))]))"
        end
    end
